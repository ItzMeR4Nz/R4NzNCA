...
-- Data skin (Nama, Label, Kelangkaan, Warna)
local skinData = {
...
-- Fungsi untuk mengatur ulang attachment beam
function rewireBeamsInContainer(container)
    local attachments = {}
    for i, descendant in ipairs(container:GetDescendants()) do
        if descendant:IsA("Attachment") and not attachments[descendant.Name] then
            attachments[descendant.Name] = descendant
        end
    end
    for i, descendant in ipairs(container:GetDescendants()) do
        if descendant:IsA("Beam") then
            local att0 = descendant.Attachment0 and descendant.Attachment0.Name
            local att1 = descendant.Attachment1 and descendant.Attachment1.Name
            if att0 and attachments[att0] then
                descendant.Attachment0 = attachments[att0]
            end
            if att1 and attachments[att1] then
                descendant.Attachment1 = attachments[att1]
            end
        end
    end
end

-- Fungsi mengganti bagian karakter
function replacePart(targetPart, sourceModel, partName)
    local targetChild = targetPart:FindFirstChild(partName)
    local sourceChild = sourceModel:FindFirstChild(partName)
    if not targetChild or not sourceChild then return end

    for i, child in ipairs(targetChild:GetChildren()) do
    ...
    pcall(function()
        targetChild.Color = sourceChild.Color
        targetChild.Material = sourceChild.Material
        targetChild.Transparency = sourceChild.Transparency
        if targetChild:IsA("UnionOperation") then
            targetChild.UsePartColor = true
        end
    end)

    local clone = sourceChild:Clone()
    clone:PivotTo(targetChild:GetPivot())
    for i, child in ipairs(clone:GetChildren()) do
        if not child:IsA("WeldConstraint") then
    ...
    for i, descendant in ipairs(targetChild:GetDescendants()) do
...
-- Fungsi utama mengaplikasikan skin
function applySupremeSkin(skinName)
    local character = player.Character
    if not character then return end

    local displayModel = workspace:FindFirstChild("BambuDisplay") and workspace.BambuDisplay:FindFirstChild("Bambu")
    if not displayModel then return end

    local skinModel = displayModel:FindFirstChild(skinName)
    if not skinModel then return end

    local backpack = workspace:FindFirstChild(player.Name) and workspace[player.Name]:FindFirstChild("BackWeapon") or character:FindFirstChild("BackWeapon")
    if not backpack then return end

    replacePart(backpack, skinModel, "Batang")
    replacePart(backpack, skinModel, "Ruas")
    rewireBeamsInContainer(backpack)
...
function applyTheme(themeName)
    ...
    -- Update semua elemen yang tersimpan di tabel uiElements
    ...
    if uiElements.SkinFrameStroke then uiElements.SkinFrameStroke.Color = theme.mid end
    if uiElements.SkinInfoText then uiElements.SkinInfoText.TextColor3 = theme.logText end
    ...
    if uiElements.FakeLabel then uiElements.FakeLabel.TextColor3 = theme.accent end
    if uiElements.FakeNameStroke then uiElements.FakeNameStroke.Color = theme.mid end
    if uiElements.StreakLabel then uiElements.StreakLabel.TextColor3 = theme.accent end
    if uiElements.StreakBoxStroke then uiElements.StreakBoxStroke.Color = theme.mid end
    ...
    if uiElements.SetNameBtn then uiElements.SetNameBtn.BackgroundColor3 = theme.primary end
    if uiElements.sbg then uiElements.sbg.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, theme.accent), ColorSequenceKeypoint.new(1, theme.mid)}) end
    if uiElements.SetStreakBtn then uiElements.SetStreakBtn.BackgroundColor3 = theme.primary end
    if uiElements.sskgb then uiElements.sskgb.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, theme.accent), ColorSequenceKeypoint.new(1, theme.mid)}) end
...
-- Membuat semua tombol tab
local tabInfo = createTabBtn("INFO", "📋", 0)
local tabMain = createTabBtn("MAIN", "⚡", 1)
local tabFake = createTabBtn("FAKE", "🎭", 2)
local tabUtil = createTabBtn("UTIL", "🔧", 3)
local tabSkin = createTabBtn("SKIN", "🎋", 4)
local tabNyawa = createTabBtn("NYAWA", "🕤", 5)
...
-- Halaman INFO
local infoPage, mainPage, fakePage, utilPage, skinPage, nyawaPage
infoPage = createPage()
...
fakePage = createPage()
fakePage.ScrollingDirection = Enum.ScrollingDirection.Y
...
skinPage = createPage()
skinPage.CanvasSize = UDim2.new(0, 0, 0, 0)
skinPage.AutomaticCanvasSize = Enum.AutomaticSize.Y
skinPage.ScrollBarThickness = isTouch and 3 or 2
skinPage.ScrollBarImageColor3 = Color3.fromRGB(150, 80, 255)
skinPage.ScrollBarImageTransparency = 0
skinPage.ScrollingDirection = Enum.ScrollingDirection.Y

local skinLayout = Instance.new("UIListLayout")
skinLayout.Padding = UDim.new(0, 8)
skinLayout.SortOrder = Enum.SortOrder.LayoutOrder
skinLayout.Parent = skinPage

local skinPad = Instance.new("UIPadding", skinPage)
skinPad.PaddingLeft = UDim.new(0, 6)
skinPad.PaddingRight = UDim.new(0, 6)
skinPad.PaddingTop = UDim.new(0, 8)
skinPad.PaddingBottom = UDim.new(0, 10)
...
function showPage(page)
    local pages = { infoPage, mainPage, fakePage, utilPage, skinPage, nyawaPage }
...
tabFake.MouseButton1Click:Connect(function()
    showPage(fakePage)
    setActiveTab(tabFake)
    playClickSound()
end)
...
-- Fungsi membuat panel speed compact
function createCompactSpeedPanel(parent, order)
    ...
    local sliderItemWidth = isTouch and 20 or 12
    ...
    for i, s in ipairs(sliders) do
        ...
        knobMin.Size = UDim2.new(0, sliderItemHeight, 0, sliderItemHeight)
        ...
        knobMax.Size = UDim2.new(0, sliderItemHeight, 0, sliderItemHeight)
        ...
        local knobRadius = sliderItemHeight / 2
        ...
        hitbox.Size = UDim2.new(1, 0, 0, sliderItemHeight * 2)
        hitbox.Position = UDim2.new(0, 0, 0.5, -sliderItemHeight)
...
createToggle("AUTO TULIS", mainPage, false, function(val) autoTulisEnabled = val if autoTulisEnabled then task.wait(0.3) gX() end end, 1)
...
uiElements.themeBtnStroke = themeBtnStroke
uiElements.themeDFStroke = themeDFStroke
...
-- Halaman FAKE
local fakePad = Instance.new("UIPadding", fakePage)
...
fakeLabel.TextColor3 = Color3.fromRGB(150, 80, 255)
fakeLabel.Font = Enum.Font.GothamBold
fakeLabel.TextSize = isTouch and 11 or 12
fakeLabel.TextXAlignment = Enum.TextXAlignment.Left
fakeLabel.Parent = fakePage
uiElements.FakeLabel = fakeLabel

-- Frame untuk input nama fake
local fakeNameFrame = Instance.new("Frame")
fakeNameFrame.Size = UDim2.new(1, 0, 0, isTouch and 70 or 80)
fakeNameFrame.LayoutOrder = 1
fakeNameFrame.BackgroundColor3 = Color3.fromRGB(14, 12, 24)
fakeNameFrame.BorderSizePixel = 0
fakeNameFrame.Parent = fakePage
Instance.new("UICorner", fakeNameFrame).CornerRadius = UDim.new(0, 12)

local fakeNameStroke = Instance.new("UIStroke", fakeNameFrame)
fakeNameStroke.Color = Color3.fromRGB(90, 40, 180)
fakeNameStroke.Thickness = 1.5
fakeNameStroke.Transparency = 0.2
uiElements.FakeNameStroke = fakeNameStroke

local fakeNameBox = Instance.new("TextBox")
fakeNameBox.Size = UDim2.new(1, -24, 0, 36)
fakeNameBox.Position = UDim2.new(0, 12, 0, 12)
fakeNameBox.BackgroundColor3 = Color3.fromRGB(25, 20, 40)
fakeNameBox.PlaceholderText = "Masukkan nama fake..."
fakeNameBox.Text = ""
fakeNameBox.TextColor3 = Color3.new(1, 1, 1)
fakeNameBox.PlaceholderColor3 = Color3.fromRGB(100, 90, 130)
fakeNameBox.Font = Enum.Font.Gotham
fakeNameBox.TextSize = isTouch and 12 or 14
fakeNameBox.ClearTextOnFocus = false
fakeNameBox.Parent = fakeNameFrame
Instance.new("UICorner", fakeNameBox).CornerRadius = UDim.new(0, 8)

local fakeNameButton = Instance.new("TextButton")
fakeNameButton.Size = UDim2.new(1, -24, 0, 28)
fakeNameButton.Position = UDim2.new(0, 12, 0, 52)
fakeNameButton.BackgroundColor3 = Color3.fromRGB(65, 20, 145)
fakeNameButton.Text = "SET NAMA"
fakeNameButton.TextColor3 = Color3.new(1, 1, 1)
fakeNameButton.Font = Enum.Font.GothamBold
fakeNameButton.TextSize = isTouch and 10 or 11
fakeNameButton.Parent = fakeNameFrame
Instance.new("UICorner", fakeNameButton).CornerRadius = UDim.new(0, 8)

local fakeNameGrad = Instance.new("UIGradient", fakeNameButton)
fakeNameGrad.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(100, 30, 220)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(65, 15, 160))
})
uiElements.SetNameBtn = fakeNameButton
uiElements.sbg = fakeNameGrad

-- Frame streak
local streakFrame = Instance.new("Frame")
streakFrame.Size = UDim2.new(1, 0, 0, isTouch and 90 or 100)
streakFrame.LayoutOrder = 2
streakFrame.BackgroundColor3 = Color3.fromRGB(14, 12, 24)
streakFrame.BorderSizePixel = 0
streakFrame.Parent = fakePage
Instance.new("UICorner", streakFrame).CornerRadius = UDim.new(0, 12)

local streakStroke = Instance.new("UIStroke", streakFrame)
streakStroke.Color = Color3.fromRGB(90, 40, 180)
streakStroke.Thickness = 1.5
streakStroke.Transparency = 0.2
uiElements.StreakBoxStroke = streakStroke

local streakLabel = Instance.new("TextLabel")
streakLabel.Size = UDim2.new(1, -24, 0, 20)
streakLabel.Position = UDim2.new(0, 12, 0, 8)
streakLabel.BackgroundTransparency = 1
streakLabel.Text = "🔥 STREAK MAKSIMAL"
streakLabel.TextColor3 = Color3.fromRGB(200, 160, 255)
streakLabel.Font = Enum.Font.GothamBold
streakLabel.TextSize = isTouch and 11 or 12
streakLabel.TextXAlignment = Enum.TextXAlignment.Left
streakLabel.Parent = streakFrame
uiElements.StreakLabel = streakLabel

local streakInput = Instance.new("TextBox")
streakInput.Size = UDim2.new(0.5, -16, 0, 36)
streakInput.Position = UDim2.new(0, 12, 0, 32)
streakInput.BackgroundColor3 = Color3.fromRGB(25, 20, 40)
streakInput.PlaceholderText = "Angka streak"
streakInput.Text = ""
streakInput.TextColor3 = Color3.new(1, 1, 1)
streakInput.PlaceholderColor3 = Color3.fromRGB(100, 90, 130)
streakInput.Font = Enum.Font.Gotham
streakInput.TextSize = isTouch and 12 or 14
streakInput.ClearTextOnFocus = false
streakInput.Parent = streakFrame
Instance.new("UICorner", streakInput).CornerRadius = UDim.new(0, 8)

local streakButton = Instance.new("TextButton")
streakButton.Size = UDim2.new(0.5, -16, 0, 36)
streakButton.Position = UDim2.new(0.5, 4, 0, 32)
streakButton.BackgroundColor3 = Color3.fromRGB(65, 20, 145)
streakButton.Text = "SET STREAK"
streakButton.TextColor3 = Color3.new(1, 1, 1)
streakButton.Font = Enum.Font.GothamBold
streakButton.TextSize = isTouch and 10 or 11
streakButton.Parent = streakFrame
Instance.new("UICorner", streakButton).CornerRadius = UDim.new(0, 8)

local streakGrad = Instance.new("UIGradient", streakButton)
streakGrad.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(100, 30, 220)),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(65, 15, 160))
})
uiElements.SetStreakBtn = streakButton
uiElements.sskgb = streakGrad

local streakInfo = Instance.new("TextLabel")
streakInfo.Size = UDim2.new(1, -24, 0, 18)
streakInfo.Position = UDim2.new(0, 12, 0, 72)
streakInfo.BackgroundTransparency = 1
streakInfo.Text = "Streak saat ini: -"
streakInfo.TextColor3 = Color3.fromRGB(150, 130, 200)
streakInfo.Font = Enum.Font.Gotham
streakInfo.TextSize = isTouch and 9 or 10
streakInfo.TextXAlignment = Enum.TextXAlignment.Left
streakInfo.Parent = streakFrame
...
local tuyulToggle = createToggle("AKTIFKAN TUYUL", tuyulFrame, false, function(val) 
    tuyulStatus.Text = val and "STATUS: ON" or "STATUS: OFF"
    tuyulStatus.TextColor3 = val and Color3.fromRGB(80, 255, 80) or Color3.fromRGB(255, 80, 80)
end, 2)
...
-- Frame skin info
local skinInfoFrame = Instance.new("Frame")
skinInfoFrame.Size = UDim2.new(1, 0, 0, isTouch and 100 or 110)
skinInfoFrame.BackgroundColor3 = Color3.fromRGB(14, 12, 24)
skinInfoFrame.BorderSizePixel = 0
skinInfoFrame.Parent = skinPage
Instance.new("UICorner", skinInfoFrame).CornerRadius = UDim.new(0, 12)

local skinInfoStroke = Instance.new("UIStroke", skinInfoFrame)
skinInfoStroke.Color = Color3.fromRGB(90, 40, 180)
skinInfoStroke.Thickness = 1.5
skinInfoStroke.Transparency = 0.2
uiElements.SkinFrameStroke = skinInfoStroke

local skinInfoTitle = Instance.new("TextLabel")
skinInfoTitle.Size = UDim2.new(1, -24, 0, 20)
skinInfoTitle.Position = UDim2.new(0, 12, 0, 8)
skinInfoTitle.BackgroundTransparency = 1
skinInfoTitle.Text = "🎋 SKIN SUPREME"
skinInfoTitle.TextColor3 = Color3.fromRGB(255, 180, 100)
skinInfoTitle.Font = Enum.Font.GothamBold
skinInfoTitle.TextSize = isTouch and 11 or 12
skinInfoTitle.TextXAlignment = Enum.TextXAlignment.Left
skinInfoTitle.Parent = skinInfoFrame

local skinInfoDesc = Instance.new("TextLabel")
skinInfoDesc.Size = UDim2.new(1, -24, 0, 40)
skinInfoDesc.Position = UDim2.new(0, 12, 0, 32)
skinInfoDesc.BackgroundTransparency = 1
skinInfoDesc.Text = "Apply skin keren ke senjatamu. Klik salah satu skin di bawah untuk mengaplikasikan."
skinInfoDesc.TextColor3 = Color3.fromRGB(170, 155, 200)
skinInfoDesc.Font = Enum.Font.Gotham
skinInfoDesc.TextSize = isTouch and 9 or 10
skinInfoDesc.TextWrapped = true
skinInfoDesc.TextXAlignment = Enum.TextXAlignment.Left
skinInfoDesc.Parent = skinInfoFrame

local skinInfoText = Instance.new("TextLabel")
skinInfoText.Size = UDim2.new(1, -24, 0, 20)
skinInfoText.Position = UDim2.new(0, 12, 0, 76)
skinInfoText.BackgroundTransparency = 1
skinInfoText.Text = "Skin aktif: -"
skinInfoText.TextColor3 = Color3.fromRGB(150, 100, 255)
skinInfoText.Font = Enum.Font.GothamBold
skinInfoText.TextSize = isTouch and 9 or 10
skinInfoText.TextXAlignment = Enum.TextXAlignment.Left
skinInfoText.Parent = skinInfoFrame
uiElements.SkinInfoText = skinInfoText
...
skinGridLayout.VerticalAlignment = Enum.VerticalAlignment.Top
skinGridLayout.FillDirectionMaxCells = 2
skinGridLayout.CellSize = UDim2.new(0.5, -6, 0, isTouch and 80 or 90)
skinGridLayout.SortOrder = Enum.SortOrder.LayoutOrder
skinGridLayout.Parent = skinGrid

-- Buat tombol skin
for i, skin in ipairs(skinData) do
    local skinBtn = Instance.new("TextButton")
    skinBtn.Size = UDim2.new(1, 0, 1, 0)
    skinBtn.LayoutOrder = i
    skinBtn.BackgroundColor3 = Color3.fromRGB(20, 18, 32)
    skinBtn.AutoButtonColor = false
    skinBtn.Parent = skinGrid
    Instance.new("UICorner", skinBtn).CornerRadius = UDim.new(0, 10)

    local btnStroke = Instance.new("UIStroke", skinBtn)
    btnStroke.Color = skin.color
    btnStroke.Thickness = 2
    btnStroke.Transparency = 0.3

    local skinName = Instance.new("TextLabel")
    skinName.Size = UDim2.new(1, 0, 0, 20)
    skinName.Position = UDim2.new(0, 0, 0, 8)
    skinName.BackgroundTransparency = 1
    skinName.Text = skin.label
    skinName.TextColor3 = Color3.new(1, 1, 1)
    skinName.Font = Enum.Font.GothamBold
    skinName.TextSize = isTouch and 12 or 14
    skinName.Parent = skinBtn

    local skinRarity = Instance.new("TextLabel")
    skinRarity.Size = UDim2.new(1, 0, 0, 16)
    skinRarity.Position = UDim2.new(0, 0, 0, 32)
    skinRarity.BackgroundTransparency = 1
    skinRarity.Text = skin.rarity
    skinRarity.TextColor3 = rarityColors[skin.rarity] or Color3.fromRGB(180, 180, 180)
    skinRarity.Font = Enum.Font.Gotham
    skinRarity.TextSize = isTouch and 9 or 10
    skinRarity.Parent = skinBtn

    local previewColor = Instance.new("Frame")
    previewColor.Size = UDim2.new(0.8, 0, 0, 8)
    previewColor.Position = UDim2.new(0.1, 0, 1, -16)
    previewColor.BackgroundColor3 = skin.color
    previewColor.BorderSizePixel = 0
    previewColor.Parent = skinBtn
    Instance.new("UICorner", previewColor).CornerRadius = UDim.new(1, 0)

    skinBtn.MouseButton1Click:Connect(function()
        playClickSound()
        skinInfoText.Text = "Skin aktif: " .. skin.label
        pcall(function() applySupremeSkin(skin.name) end)
    end)
...
-- Dropdown skin
local skinDropBtn = Instance.new("TextButton")
skinDropBtn.Size = UDim2.new(1, -4, 0, TOGGLE_HEIGHT)
skinDropBtn.BackgroundColor3 = Color3.fromRGB(65, 20, 145)
skinDropBtn.Text = "🎨 PILIH TEMA WARNA"
skinDropBtn.TextColor3 = Color3.new(1, 1, 1)
skinDropBtn.Font = Enum.Font.GothamBold
skinDropBtn.TextSize = TEXT_SIZE_SMALL
skinDropBtn.Parent = skinPage
Instance.new("UICorner", skinDropBtn).CornerRadius = UDim.new(0, 9)

local skinDropGrad = Instance.new("UIGradient", skinDropBtn)
...
uiElements.SkinDropBtn = skinDropBtn
uiElements.SkinDropGrad = skinDropGrad
...
-- Fungsi untuk drag window (hanya jika tidak di HP)
if not isTouch then
