...
local function createTabButton(tabName, pos)
    ...
    local btnWidth = math.min(120, (TabContainer.AbsoluteSize.X - 20) / #tabs)
...
local function refreshItemESP()
    ...
    for _, child in ipairs(droppedItemsFolder:GetChildren()) do
        if table.find(finalItemNames, child.Name) then
            createItemESP(child)
        end
    end
end
...
if droppedItemsFolder then
    local childAddedConn = droppedItemsFolder.ChildAdded:Connect(function(child)
        if itemOptions.ESP and table.find(finalItemNames, child.Name) then
            createItemESP(child)
        end
    end)
    table.insert(connections, childAddedConn)
        ...
        humanoid:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
            if speedState and humanoid.WalkSpeed ~= targetSpeed then
                ...
                if speedState then
                    ...
                    task.wait(2)
                    if StatusText and not StatusText.Text:find("Speed auto-fixed") then
                    end
                end
            end
        end)
    ...
    if char then
        ...
        if humanoid then
            if not speedState then
                ...
                StatusText.Text = "Speed boost aktif - Auto fix enabled"
                setupSpeedFix()
            else
                ...
                TweenService:Create(speedBtn, TweenInfo.new(0.2), {BackgroundColor3 = speedNormalColor}):Play()
                StatusText.Text = "Speed boost nonaktif"
                if speedFixConnection then
                    speedFixConnection:Disconnect()
...
yOffset = yOffset + (TouchEnabled and 44 or 40)

local speedValueLabel = Instance.new("TextLabel")
speedValueLabel.Size = UDim2.new(0, 80, 0, 25)
speedValueLabel.Position = UDim2.new(0, 5, 0, yOffset)
...
speedSlider.Position = UDim2.new(0, 100, 0, yOffset)
...
speedSlider.FocusLost:Connect(function()
    ...
    if newSpeed and newSpeed > 0 then
        ...
        if speedState then
            applySpeed()
            StatusText.Text = "Speed diubah ke: " .. targetSpeed
        end
    else
...
local autoPickupGroup, autoPickupContainer = createGroup(exploits, "Auto Pickup", yOffset, 12)
...
local modeNames = {"Langsung", "Ke Ransel", "Keduanya"}
local modeColors = {
...
local _autoPickupBtn, _autoPickupState = createToggleButton(autoPickupContainer, "Auto Pickup", false, function(state)
    ...
    if state then
        StatusText.Text = "Auto Pickup gestartet - Mode: " .. modeNames[pickupModeType]
        startAutoPickup()
    else
...
yOffset = 5
createInfoLabel("Author: Spy", Color3.fromRGB(255, 100, 100), TouchEnabled and 16 or 18, yOffset)
yOffset = yOffset + 30
createInfoLabel("Discord: hxnrylsd", Color3.fromRGB(255, 255, 255), TouchEnabled and 14 or 16, yOffset)
yOffset = yOffset + 30
createInfoLabel("Version: 5.0", Color3.fromRGB(255, 255, 255), TouchEnabled and 14 or 16, yOffset)
yOffset = yOffset + 30
createInfoLabel("My Socials: feds.lol/spy", Color3.fromRGB(255, 255, 255), TouchEnabled and 14 or 16, yOffset)
yOffset = yOffset + 35
createInfoLabel("Größte KI-Cheat, den ich je gesehen habe!", Color3.fromRGB(255, 200, 200), TouchEnabled and 13 or 15, yOffset)
yOffset = yOffset + 30
createInfoLabel("Created with my favorite Slave, Deepseek.", Color3.fromRGB(200, 200, 255), TouchEnabled and 13 or 15, yOffset)
yOffset = yOffset + 40
...
print("Total Items loaded: " .. #finalItemNames)