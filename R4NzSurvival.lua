--[[
    ...
    - Auto Pickup (Food only to Backpack, Others to Inventory)
...
local itemNames = {
    ...
    -- Food & Drinks (hanya bisa ke ransel)
...
-- Daftar item makanan (hanya bisa pickup jika ada ransel)
local foodItems = {
    "Beans", "Bloxiade", "Bloxy Cola", "Water Bottle", "Canned Food", "MRE", "Energy Drink"
}
...
-- Fungsi untuk cek apakah player memegang ransel
...
-- ========== Exploits Tab mit Smart Auto Pickup ==========
...
radiusInput.Position = UDim2.new(0, 90, 0, 48)
...
delayLabel.Position = UDim2.new(0, 5, 0, 85)
...
delayInput.Position = UDim2.new(0, 90, 0, 83)
...
allItemsCheckbox.Position = UDim2.new(0, 5, 0, 120)
...
itemListFrame.Position = UDim2.new(0, 5, 0, 160)
...
local _autoPickupBtn, _autoPickupState = createToggleButton(autoPickupContainer, "Auto Pickup", false, function(state)
    ...
    if state then
        StatusText.Text = "Auto Pickup started"
        startAutoPickup()
    else
                ...
                if dist <= radius then
                    ...
                    if isFoodItem(item.Name) then
                        ...
                        if hasBackpack() then
                            ...
                            -- Tidak ada ransel, skip makanan
                            -- StatusText.Text = "Need backpack for food items"
                        end
                    else
                        -- Non-makanan: pickup ke inventory
...
print("Features: ESP (Mob/Item with Filter) | Smart Auto Pickup | Speed Hack with Auto Fix")
print("Features: Infinite Jump | Auto Drop Ammo")
print("Smart Pickup: Food items only go to BACKPACK, other items go to INVENTORY")