...
local function hasBackpack()
    ...
    for _, tool in ipairs(char:GetChildren()) do
        if tool:IsA("Tool") and (tool.Name:lower():find("backpack") or tool.Name:lower():find("bag") or tool.Name:lower():find("pack")) then
            return true
        end
    end
    ...
    if itemOptions.Name or itemOptions.Distance then
        ...
        local connection = RunService.RenderStepped:Connect(function()
            ...
            if distLabel and distLabel.Visible then
                ...
                local myRoot = myChar and (myChar:FindFirstChild("HumanoidRootPart") or myChar:FindFirstChild("Torso") or myChar:FindFirstChild("UpperTorso"))
                if myRoot then
                    distLabel.Text = math.floor((myRoot.Position - mainPart.Position).Magnitude) .. "m"
                end
            end
        end)
        espTable.DistanceConnection = connection
...
createToggleButton(mobContainer, "Mob ESP", mobOptions.ESP, function(s) mobOptions.ESP = s refreshMobESP() StatusText.Text = s and "Mob ESP enabled" or "Mob ESP disabled" end, 0, 140)
local subY = 40
...
createToggleButton(itemContainer, "Item ESP", itemOptions.ESP, function(s) itemOptions.ESP = s refreshItemESP() StatusText.Text = s and "Item ESP enabled" or "Item ESP disabled" end, 0, 140)
subY = 40
...
-- ========== Exploits Tab dengan 3 Mode + Filter Makanan ==========
...
infoLabel.Text = "⚠️ FOOD ITEMS: Only pickup if you have BACKPACK\n   Non-Food items: Pickup normally"
...
pickupMode.Position = UDim2.new(0, 90, 0, 48)
...
radiusLabel.Position = UDim2.new(0, 5, 0, 90)
...
radiusInput.Position = UDim2.new(0, 90, 0, 88)
...
delayLabel.Position = UDim2.new(0, 5, 0, 125)
...
delayInput.Position = UDim2.new(0, 90, 0, 123)
...
-- MAIN AUTO PICKUP FUNCTION (3 Mode + Food Filter)
    ...
    autoPickupConnection = RunService.Heartbeat:Connect(function()
        ...
        for _, item in ipairs(droppedItemsFolder:GetChildren()) do
            ...
            if mainPart then
                ...
                if dist <= radius then
                    -- CEK: Apakah item makanan?
                    if isFoodItem(item.Name) then
                        -- Makanan: HANYA bisa di-pickup jika ada ransel
                        if hasBackpack() then
                        ...
                        -- Jika tidak ada ransel, makanan tidak dipickup (skip)
                    else
                        -- Non-makanan: pickup normal sesuai mode yang dipilih
                        if pickupModeType == 1 then
                            -- Mode Direct: pickup ke inventory
                            if pickUpItemRemote then
                                pickUpItemRemote:FireServer(item)
                                task.wait(delay)
                            end
                        elseif pickupModeType == 2 then
                            -- Mode Backpack: pickup ke ransel
                            if adjustBackpackRemote then
                                adjustBackpackRemote:FireServer(item)
                                task.wait(delay)
                            end
                        elseif pickupModeType == 3 then
                            -- Mode Both: coba ransel dulu, gagal ke inventory
                            local picked = false
                            if adjustBackpackRemote then
                                adjustBackpackRemote:FireServer(item)
                                picked = true
...
local function dropAmmo(ammoItem, amount)
    ...
    if ammoCount then
        local toDrop = amount
        if amount == 0 then
            toDrop = ammoCount.Value
        else
            toDrop = math.min(amount, ammoCount.Value)
        end
        if toDrop > 0 then
...
amountInput.FocusLost:Connect(function()
    ...
    if value and value >= 0 then dropAmount = value else dropAmount = 0 amountInput.Text = "0" end
end)
...
local statusConn = RunService.RenderStepped:Connect(function()
    ...
    StatusText.Text = string.format("Ready | FPS: %d | Mob: %s | Item: %s | Jump: %s | Auto: %s | Drop: %s",
        fps,
        ...
        autoDropActive and "On" or "Off")
end)
table.insert(connections, statusConn)

print("SPYMM v5.0 - Full Features Loaded!")
print("Press Insert to toggle GUI")
print("Auto Pickup Rules:")
print("- Mode Direct: Non-Food items only (Food needs backpack)")
print("- Mode Backpack: All items to backpack")
print("- Mode Both: Try backpack first, then inventory")