--[[
    SPYMM v8.3 - Exploits Only (Dengan Floating Button Auto Pickup)
    Ukuran icon kecil 45x45, drag sensitifitas rendah
]]
...
-- ============================================
-- FLOATING BUTTON (Ukuran 45x45, drag sensitifitas rendah)
-- ============================================
local function createFloatingButton()
    ...
    -- Tombol utama (ukuran 45x45 - sama seperti icon R4NzDev)
    ...
    -- Corner (bikin jadi bulat sempurna)
    ...
    -- Variabel drag (sensitifitas RENDAH)
    local isDragging = false
    local dragStartPos = nil
    local buttonStartPos = nil
    local lastDragDelta = 0
    local dragThreshold = 5  -- Jarak minimal untuk memulai drag (mencegah klik ke-drag)
    ...
    button.MouseLeave:Connect(function()
        if not Toggles.AutoPickup or not Toggles.AutoPickup.Value then
    ...
    -- Fungsi drag dengan sensitifitas RENDAH
    ...
    UserInputService.InputChanged:Connect(function(input)
        if dragActive and (input.UserInputType == Enum.UserInputType.MouseMovement or 
           input.UserInputType == Enum.UserInputType.Touch) then
            ...
            lastDragDelta = math.abs(delta.X) + math.abs(delta.Y)
            
            -- Hanya gerakkan jika drag melebihi threshold (sensitifitas RENDAH)
            if lastDragDelta > dragThreshold then
                local newX = buttonStartPos.X.Scale + (delta.X / game:GetService("CoreGui").AbsoluteSize.X) * 0.8
                local newY = buttonStartPos.Y.Scale + (delta.Y / game:GetService("CoreGui").AbsoluteSize.Y) * 0.8
    ...
    UserInputService.InputEnded:Connect(function(input)
        if dragActive and (input.UserInputType == Enum.UserInputType.MouseButton1 or 
           input.UserInputType == Enum.UserInputType.Touch) then
            -- Jika jarak drag kecil (< threshold), anggap sebagai klik, bukan drag
            if lastDragDelta <= dragThreshold then
                -- Klik toggle
