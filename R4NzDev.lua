...
-- Variabel untuk auto answer (FITUR BARU)
local Sx = false  -- Mode manusia (salah ketik)
local tx = false  -- Mode tuyul (auto beruntun)
local JX = 3      -- Jumlah target jawab dalam mode tuyul
local qX = 0      -- Counter jawaban dalam mode tuyul
local NX = false  -- Status mode tuyul aktif
local WX = 0      -- Counter spam dalam mode tuyul
local vX = 2      -- Versi mode tuyul
local pX = false  -- Mode blatant
local ZX = ""     -- Menyimpan kata yang akan dikirim (blatant)
local eX = ""     -- Menyimpan awalan (blatant)

-- Database kata random (cadangan)
local kataRandom = { "wkwk", "receh", "noob", "lemah", "kasian", "santuy", "ezz", "wkwk", "lol", "mudah" }

-- Database kata per kategori (akan diisi dari GitHub)
local dropData = { 
...
local MX = {}  -- Mastered Words (kata yang sudah pernah dipakai)

-- Database kata umum (akan diisi dari GitHub)
local zX = {
    -- Contoh kata-kata sementara (akan ditimpa oleh data dari GitHub)
...
-- Fungsi untuk mengetik dengan simulasi manusia
function simulateTyping(text, totalLength)
    if not isRunning then return end
    
    -- Simpan total panjang kata (untuk keperluan delete nanti)
    totalLength = totalLength or #text
    
    -- Mode manusia: simulasi salah ketik dan koreksi
    if Sx then
        local errorCount = math.random(1, 2)  -- Jumlah kesalahan (1-2 kali)
        local errorsDone = 0
        
        -- Kadang-kadang jeda dulu sebelum mulai ngetik
        ...
        while i <= #text do
            ...
            -- Simulasi salah ketik (mengetik huruf random)
            ...
            -- Simulasi salah ketik lalu langsung dihapus (tanpa lanjut ngetik)
        ...
        -- Kadang-kadang setelah selesai ngetik, malah ngetik kata random lalu dihapus
        ...
        -- Mode normal (tanpa simulasi salah ketik)
        for i = 1, #text do
            local char = string.upper(string.sub(text, i, i))
            local keyCode = Enum.KeyCode[char]
...
-- Variabel untuk mencegah eksekusi ganda
local LX = false  -- Lock status
local bX = false  -- Rejoin lock

-- FUNGSI UTAMA AUTO ANSWER (gX)
gX = function()
    -- Cek apakah auto tulis enabled dan tidak dalam status lock
    ...
    -- CEK MODE TUYUL (auto beruntun)
    if tx and NX then
        -- Deteksi huruf awalan dari UI
        ...
        -- Jika awalan ditemukan
        if awalan ~= "" then
            if uiElements.LogAwalan then uiElements.LogAwalan.Text = "👻 SPAM " .. (WX + 1) .. "/2" end
            
            -- Ketik awalan (huruf yang sudah ada di kotak jawaban)
            for i = 1, #awalan do
            ...
            -- Ketik huruf random sebagai "jawaban" (mode tuyul = asal-asalan)
    ...
    -- ==============================================
    -- MODE NORMAL (Mencari jawaban berdasarkan awalan)
    -- ==============================================
    ...
    if awalan ~= "" then
        ...
        -- KUMPULKAN KATA YANG COCOK DARI SEMUA KATEGORI YANG DIPILIH
        local candidates = {}
        local candidatesFromCat = {}
        
        -- Ambil kata dari kategori yang dipilih
        ...
        -- Prioritaskan kata dari kategori yang dipilih
        if #candidatesFromCat > 0 then
            candidates = candidatesFromCat
        end
        
        -- Jika tidak ada dari kategori, ambil dari database umum
        ...
        -- Jika ada kandidat kata
        if #candidates > 0 then
            -- Pilih kata secara acak
            local selectedWord = candidates[math.random(1, #candidates)]
            
            -- Cari tahu dari kategori mana kata ini berasal (untuk log)
            ...
            -- Simpan kata yang dipilih untuk keperluan tracking
            local mX = selectedWord
            
            -- KETIK SISA HURUF (karena huruf awalan sudah ada di kotak)
            simulateTyping(string.sub(selectedWord, #awalan + 1), #selectedWord)
            
            -- Tandai kata ini sebagai sudah dipakai
            MX[selectedWord] = true
            
            -- Update counter mode tuyul jika aktif
            ...
            -- Tunggu sebentar lalu reset log
            task.wait(1)
...
-- Load kata per kategori dari GitHub
task.spawn(function()
    local categoryUrls = {
...
-- Load database umum dari GitHub (KAMUS UTAMA)
task.spawn(function()
    ...
    if success and (type(response) == "string" and #response > 1000) then
        ...
        -- Baca file baris per baris dan masukkan ke tabel zX
        for word in string.gmatch(response, "[^\r\n]+") do
            local cleanWord = (word:gsub("%s+", "")):lower()
            -- Hanya masukkan kata yang lebih dari 1 huruf dan hanya mengandung huruf
            if #cleanWord > 1 and string.match(cleanWord, "^%a+$") then
                table.insert(zX, cleanWord)
            end
        end
        
        -- Acak urutan kata agar tidak monoton
        for i = #zX, 2, -1 do
...
-- Tambahkan toggle untuk MODE MANUSIA (simulasi salah ketik)
createToggle("🕠 MANUSIA [FIX]", mainPage, false, function(val) 
    Sx = val 
end, 32)

-- Tambahkan toggle untuk MODE TUYUL
...
-- Label untuk menampilkan awalan
...
-- Label untuk menampilkan kata yang akan diketik
...
-- ==============================================
-- HALAMAN UTIL (TAMBAHKAN STATUS TUYUL)
-- ==============================================
...
-- Toggle untuk mengaktifkan tuyul mode
...
-- ==============================================
-- FITUR DRAG WINDOW (GESER GUI) - SUDAH BENAR
-- ==============================================
...
-- Fungsi untuk mengupdate posisi saat drag
...
-- ==============================================
-- RESIZE WINDOW (SUDAH ADA SEBELUMNYA)
-- ==============================================
