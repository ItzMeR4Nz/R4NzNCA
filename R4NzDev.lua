...
function addGameLog(question, answer, status)
    ...
    table.insert(gameLogs, logEntry)
    updateLogDisplay()
end

function resetCurrentGameLogs()
    currentGameLogs = {}
    updateLogDisplay()
end
...
function updateLogDisplay()
    ...
    for i, log in ipairs(currentGameLogs) do
        ...
        logItem.Size = UDim2.new(1, -10, 0, 40)
        logItem.Position = UDim2.new(0, 5, 0, yPos)
        logItem.BackgroundColor3 = i % 2 == 0 and Color3.fromRGB(20, 18, 32) or Color3.fromRGB(25, 22, 40)
        ...
        questionLabel.Size = UDim2.new(0.4, -10, 1, 0)
        questionLabel.Position = UDim2.new(0, 55, 0, 0)
        questionLabel.BackgroundTransparency = 1
        questionLabel.Text = log.question:upper()
        questionLabel.TextColor3 = Color3.fromRGB(200, 160, 255)
        ...
        answerLabel.Size = UDim2.new(0.4, -10, 1, 0)
        answerLabel.Position = UDim2.new(0.4, 50, 0, 0)
        answerLabel.BackgroundTransparency = 1
        answerLabel.Text = log.answer:upper()
        answerLabel.TextColor3 = log.status == "✓" and Color3.fromRGB(80, 255, 80) or Color3.fromRGB(255, 80, 80)
        answerLabel.Font = Enum.Font.GothamBold
        answerLabel.TextSize = 11
        ...
        statusLabel.TextColor3 = log.status == "✓" and Color3.fromRGB(80, 255, 80) or Color3.fromRGB(255, 80, 80)
    ...
    logScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, yPos + 5)
end

function getLogsText(mode)
    local logs = (mode == "current") and currentGameLogs or gameLogs
    ...
    text = text .. "\nTotal: " .. #logs .. " pertanyaan"
    return text
end

function copyLogsToClipboard(mode)
    local text = getLogsText(mode)
    setclipboard and setclipboard(text) or writeclipboard and writeclipboard(text)
...
copyCurrentBtn.MouseButton1Click:Connect(function()
    playClickSound()
    copyLogsToClipboard("current")
end)

copyAllBtn.MouseButton1Click:Connect(function()
    playClickSound()
    copyLogsToClipboard("all")
end)
...
-- Halaman SKIN dan FAKE TELAH DIHAPUS

-- Dropdown skin (TEMA WARNA) - TETAP ADA
