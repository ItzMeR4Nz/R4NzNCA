...
-- ============================================================
-- POLA AKHIRAN KATA SULIT (didefinisikan DULUAN sebelum load kamus)
-- ============================================================
...
-- FIX BUG 10: Hapus "SEMUA KATA EKSTRIM" dari dropData & dropState
local dropData = {
...
-- FIX BUG 10: "SEMUA KATA EKSTRIM" dihapus dari dropState
local dropState = {
...
-- ============================================================
-- FIX BUG 2: Load kamus SETELAH polaAkhiranKat didefinisikan
-- Semua kata sulit difilter dari kamus.txt (tidak perlu URL terpisah)
-- ============================================================
task.spawn(function()
    ...
    if success and type(res) == "string" and #res > 1000 then
        ...
        -- Filter kata sulit langsung dari kamus (tidak perlu request terpisah)
        for cat, pola in pairs(polaAkhiranKat) do
            dropData[cat] = {}
            for _, word in ipairs(zX) do
                if word:match(pola) then
                    table.insert(dropData[cat], word)
                end
            end
            print("✅ Filter "..cat..": "..#dropData[cat].." kata")
        end

        -- SEMUA KATA SULIT = gabungan unik semua kategori
        local sudahAda = {}
        dropData["SEMUA KATA SULIT"] = {}
        for _, cat in ipairs({
            "IF","X","NG","AI","KS","CY","UI","LY","GY",
            "LT","EO","OE","EKS","OO","KN","Q","MP","SF","TT"
        }) do
            for _, word in ipairs(dropData[cat] or {}) do
                if not sudahAda[word] then
                    sudahAda[word] = true
                    table.insert(dropData["SEMUA KATA SULIT"], word)
                end
            end
        end
        print("✅ SEMUA KATA SULIT: "..#dropData["SEMUA KATA SULIT"].." kata")
...
-- FIX BUG 4: MX hanya direset di newMatch(), tidak di gX()
...
-- FIX BUG 8: Feedback error jika setclipboard tidak tersedia
local function copyLiveLog()
    ...
    if setclipboard then
        ...
        -- FIX BUG 8: Tampilkan feedback error jika tidak tersedia
        copyLogBtn.Text = "✗"
        copyLogBtn.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
        warn("⚠️ setclipboard tidak tersedia di executor ini")
    end
...
-- ============================================================
-- FIX BUG 11: startInstantRejoin didefinisikan DULU sebelum remotes connect
-- ============================================================
...
-- ============================================================
-- FIX BUG 1 & 6: detectAwalan() dengan filter duplikat & validasi kamus
-- ============================================================
local function detectAwalan()
    ...
    local candidates = {}
    local sudahAda = {}  -- FIX BUG 6: filter duplikat
    ...
    for _, obj in pairs(playerGui:GetDescendants()) do
        if obj:IsA("TextLabel") and obj.Visible then
            ...
            if raw then
                ...
                if #cleaned >= 1 and #cleaned <= 4
                   and cleaned:match("^[a-z]+$")
                   and not sudahAda[cleaned] then  -- FIX BUG 6: skip duplikat

                    local absSize = obj.AbsoluteSize
                    if absSize.X >= 20 and absSize.Y >= 14 then
                        sudahAda[cleaned] = true
                        table.insert(candidates, {
                            text = cleaned,
                            size = absSize.X * absSize.Y,
                            fontSize = obj.TextSize or 14
                        })
                    end
                end
            end
    ...
    -- Prioritaskan font terbesar (soal biasanya ditampilkan besar)
    table.sort(candidates, function(a, b)
        if a.fontSize ~= b.fontSize then
            return a.fontSize > b.fontSize
        end
        return a.size > b.size
    end)

    -- Validasi: pastikan ada kata di kamus yang berawalan sama
    for i = 1, math.min(#candidates, 5) do
        local cand = candidates[i].text
        local panjang = #cand
        for _, word in ipairs(zX) do
            if #word > panjang and word:sub(1, panjang) == cand then
                return cand  -- valid, langsung return
            end
        end
    end
...
-- ============================================================
-- FIX BUG 1: cariKataByAwalan() return konsisten pool + label
-- ============================================================
local function cariKataByAwalan(awalan)
    ...
    -- FIX: Validasi kata ketat - awalan harus cocok PERSIS
    local function valid(word)
        if #word <= panjang then return false end
        if word:sub(1, panjang) ~= awalan then return false end
        if MX[word] then return false end
        if not word:match("^[a-z]+$") then return false end
        return true
    end

    -- Cari dari kategori yang diaktifkan
    ...
    -- Pilih pool berdasarkan mode
    local pool, label
    if skakmatEnabled then
        if #skakmatCat > 0 then
            pool, label = skakmatCat, "SKAKMAT★"
        elseif #skakmatKBB > 0 then
            pool, label = skakmatKBB, "SKAKMAT"
        elseif #biasaCat > 0 then
            pool, label = biasaCat, "CAT"
        else
            pool, label = biasaKBB, "KBB"
        end
    else
...
-- ============================================================
-- FIX BUG 5: gX() dibungkus pcall agar LX selalu di-reset
-- FIX BUG 1: Tangani return nil dari cariKataByAwalan
-- FIX BUG 3: Guard sisaHuruf kosong
-- FIX BUG 4: Tidak ada MX={} di sini, hanya di newMatch()
-- ============================================================
local function gX()
    if not autoTulisEnabled or not isRunning or LX then return end
    LX = true

    -- FIX BUG 5: Bungkus seluruh logika dengan pcall agar LX pasti direset
    local ok, err = pcall(function()
        ...
        -- FIX BUG 1: Tangani return nil/nil
        local candidates, srcLabel = cariKataByAwalan(awalan)
        ...
        local srcTag = picked.src or srcLabel or "KBB"

        -- FIX BUG 3: Guard sisaHuruf kosong
        local sisaHuruf = selectedWord:sub(#awalan + 1)
        if sisaHuruf == "" then
            print("⚠️ Kata sama panjang dengan awalan, skip: "..selectedWord)
    ...
    -- FIX BUG 5: LX SELALU direset, apapun yang terjadi (error atau normal)
    if not ok then
        warn("❌ Error di gX():", err)
        if uiElements.LogAwalan then uiElements.LogAwalan.Text = "AWALAN: -" end
        if uiElements.LogIsi then uiElements.LogIsi.Text = "-" end
    end
...
-- Loop utama (1.5 detik, dibiarkan sesuai permintaan)
task.spawn(function()
    while isRunning do
...
-- ============================================================
-- REMOTE EVENT (FIX BUG 11: startInstantRejoin sudah didefinisikan di atas)
-- ============================================================
...
2. Cari kata berawalan sama di kamus
...
✅ SEMUA KATA SULIT:
Difilter langsung dari kamus.txt
Tidak perlu load file terpisah
...
-- FIX BUG 9: Guard mainFrame nil di RenderStepped
...
local ok, err = pcall(function()
    ...
    print("🔑 Semua bug telah diperbaiki")
