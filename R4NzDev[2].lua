...
-- Fungsi mengaplikasikan tema (disederhanakan)
...
-- Database utama
local database = {
    belakang = {},  -- Untuk kata berakhiran
    depan = {}      -- Untuk kata berawalan (pakai zX nanti)
}
...
-- Set database.depan = zX
database.depan = zX

-- URL database dari GitHub
local categoryUrls = {
...
-- Mapping kategori ke pola akhiran
local polaAkhiran = {
    IF = "if$",
    X = "x$",
    NG = "ng$",
...
-- Load database dari GitHub (otomatis pilah)
task.spawn(function()
            ...
            if success and type(response) == "string" then
                ...
                for word in string.gmatch(response, "[^\r\n]+") do
                    ...
                    if #cleanWord > 1 and string.match(cleanWord, "^%a+$") then
                        -- Untuk SEMUA KATA SULIT, masukkan semua
                        ...
                        -- Untuk kategori lain, filter sesuai pola akhiran
...
-- ==============================================
-- LIVE LOG FLOATING (SELALU KELIATAN)
-- ==============================================
...
function updateLogDisplayFloating()
    ...
    if #currentGameLogs == 0 then
        ...
        emptyLabel.Text = "Belum ada log\nMatch #" .. matchCounter
        emptyLabel.TextColor3 = Color3.fromRGB(150, 150, 150)
        emptyLabel.Font = Enum.Font.Gotham
        emptyLabel.TextSize = 10
        emptyLabel.TextWrapped = true
        emptyLabel.Parent = logFloatScroll
        logFloatScroll.CanvasSize = UDim2.new(0, 0, 0, 50)
        return
    end
    ...
    for i, log in ipairs(currentGameLogs) do
        ...
        if log.tipe == "depan" then
            bgColor = Color3.fromRGB(40, 30, 50)  -- Warna beda untuk depan
        end
        
        local logItem = Instance.new("Frame")
        logItem.Size = UDim2.new(1, 0, 0, 28)
        logItem.Position = UDim2.new(0, 0, 0, yPos)
        ...
        qLabel.Size = UDim2.new(0, 25, 1, 0)
        qLabel.Position = UDim2.new(0, 50, 0, 0)
        qLabel.BackgroundTransparency = 1
        qLabel.Text = log.question
        qLabel.TextColor3 = Color3.fromRGB(200, 180, 255)
        ...
        aLabel.Size = UDim2.new(1, -110, 1, 0)
        aLabel.Position = UDim2.new(0, 80, 0, 0)
        aLabel.BackgroundTransparency = 1
        aLabel.Text = string.sub(log.answer, 1, 8) .. (log.answer:len() > 8 and "..." or "")
        aLabel.TextColor3 = log.status == "✓" and Color3.fromRGB(100, 255, 100) or Color3.fromRGB(255, 100, 100)
        ...
        typeLabel.Position = UDim2.new(1, -22, 0, 0)
        typeLabel.BackgroundTransparency = 1
        typeLabel.Text = log.tipe == "depan" and "⬆️" or "⬇️"
        typeLabel.TextColor3 = log.tipe == "depan" and Color3.fromRGB(255, 200, 100) or Color3.fromRGB(100, 200, 255)
    ...
    logFloatScroll.CanvasSize = UDim2.new(0, 0, 0, yPos + 5)
end
...
function copyLiveLog()
    ...
    for i, log in ipairs(currentGameLogs) do
        textToCopy = textToCopy .. string.format("%s |  %s   | %-12s | %s\n", 
            log.time, log.question, log.answer, log.tipe == "depan" and "DEPAN" or "BELAKANG")
    end
...
-- Status toggle per kategori (LENGKAP!)
local dropState = { 
    ...
    ["SEMUA KATA EKSTRIM"] = false  -- TOMBOL BARU!
}

-- Database kata per kategori (dari GitHub nanti)
local dropData = { 
    IF = {}, X = {}, NG = {}, AI = {}, KS = {}, CY = {}, UI = {}, 
    LY = {}, GY = {}, LT = {}, EO = {}, OE = {}, EKS = {}, OO = {}, 
    KN = {}, Q = {}, MP = {}, SF = {}, TT = {}, 
    SEMUA = {}, ["SEMUA KATA SULIT"] = {} 
}
dropData["SEMUA KATA EKSTRIM"] = {} -- Akan diisi dari semua kategori

-- MX untuk kata yang sudah dipakai
local MX = {}

-- Fungsi cari kata berdasarkan AKHIRAN (FOKUS UTAMA!)
function cariBerdasarkanAkhiran(soal, kategori)
    ...
    for _, word in ipairs(databaseKategori) do
        -- CEK AKHIRAN: ambil panjangSoal huruf terakhir
...
-- Fungsi cari kata berdasarkan AWALAN (cadangan)
...
-- Fungsi utama auto answer (HEART OF THE SCRIPT!)
function gX()
    ...
    if awalan ~= "" then
        -- Tentukan mode berdasarkan analisa
        local total = systemPatterns.pattern.depan + systemPatterns.pattern.belakang
        local pakeBelakang = true  -- Default belakang
        
        if total >= 5 then
            -- Kalau udah cukup data, ikutin pola yang paling sering
            pakeBelakang = systemPatterns.pattern.belakang > systemPatterns.pattern.depan
        end
        
        local candidates = {}
        local tipe = pakeBelakang and "belakang" or "depan"
        
        -- Cari kata berdasarkan mode
        if pakeBelakang then
            -- FOKUS AKHIRAN (INI YANG ANDA MINTA!)
            for cat, enabled in pairs(dropState) do
                if enabled then
                    local catCandidates = cariBerdasarkanAkhiran(awalan, cat)
                    for _, word in ipairs(catCandidates) do
                        table.insert(candidates, word)
                    end
                end
            end
        else
            -- PAKAI AWALAN (cadangan, jarang dipakai)
            candidates = cariBerdasarkanAwalan(awalan)
        end
        
        -- Kalau gak ketemu di kategori, coba database umum
        if #candidates == 0 then
            if pakeBelakang then
                -- Coba di SEMUA KATA SULIT
                for _, word in ipairs(database.belakang.SEMUA or {}) do
                    if string.sub(word, -#awalan) == awalan and not MX[word] then
                        table.insert(candidates, word)
                    end
                end
            else
                -- Coba di zX (kamus umum)
                candidates = cariBerdasarkanAwalan(awalan)
            end
        end
        
        -- Kalau masih kosong, generate random (emergency)
        if #candidates == 0 and pakeBelakang then
            -- Generate kata random dengan akhiran sesuai soal
            local chars = "abcdefghijklmnopqrstuvwxyz"
            local randomWord = ""
            for i = 1, 3 do
                randomWord = randomWord .. string.sub(chars, math.random(1, #chars), math.random(1, #chars))
            end
            randomWord = randomWord .. awalan
            table.insert(candidates, randomWord)
        end
        
        if #candidates > 0 then
            local selectedWord = candidates[math.random(1, #candidates)]
            
            print("🤖 R4NzDev EKSTRIM: " .. selectedWord:upper() .. " | Soal: " .. awalan:upper() .. " | Mode: " .. tipe)
            
            -- Tambah ke log
            addGameLog(awalan, selectedWord, "✓", tipe)
            
            -- Tandai sudah dipakai
            MX[selectedWord] = true
            
            -- Simulasi ngetik (sisa huruf setelah soal)
            local kataTulis = string.sub(selectedWord, #awalan + 1)
            simulateTyping(kataTulis, #selectedWord)
        end
    end
...
-- ==============================================
-- DROPDOWN KATEGORI (DENGAN TOMBOL EKSTRIM)
-- ==============================================
...
-- Daftar kategori LENGKAP dengan EKSTRIM
local categories = { 
    "IF", "X", "NG", "AI", "KS", "CY", "UI", "LY", "GY", "LT", 
    "EO", "OE", "EKS", "OO", "KN", "Q", "MP", "SF", "TT", 
    "SEMUA KATA SULIT", 
    "SEMUA KATA EKSTRIM"  -- TOMBOL BARU!
}
...
function refreshDropUI()
    ...
    for i, cat in ipairs(categories) do
        ...
        local isAllSelected = (dropState["SEMUA KATA SULIT"] or dropState["SEMUA KATA EKSTRIM"]) and 
                              cat ~= "SEMUA KATA SULIT" and cat ~= "SEMUA KATA EKSTRIM"
        local isSelected = dropState[cat]

        catBtn.BackgroundColor3 = isAllSelected and Color3.fromRGB(25, 22, 36) or 
                                  isSelected and Color3.fromRGB(80, 30, 170) or 
                                  Color3.fromRGB(28, 25, 42)
        
        -- Warna khusus untuk EKSTRIM
        ...
        catBtn.TextColor3 = isAllSelected and Color3.fromRGB(70, 65, 90) or 
                            isSelected and Color3.new(1, 1, 1) or 
        ...
        catBtn.MouseButton1Click:Connect(function()
            ...
            elseif cat == "SEMUA KATA EKSTRIM" then
                ...
                if dropState["SEMUA KATA EKSTRIM"] then
                    -- AKTIFKAN SEMUA KATEGORI EKSTRIM!
...
UserInputService.InputChanged:Connect(function(input)
    ...
    logFloating.Position = UDim2.new(1, -290, 0, 10) -- biar tetap di pojok
end)
...
UserInputService.InputChanged:Connect(function(input, processed)
    if isResizing and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        -- Update posisi glow dan resize handle
...
-- Sinkronisasi posisi resize handle setiap frame (optional)
game:GetService("RunService").RenderStepped:Connect(function()
...
-- ==============================================
-- DEBUG OVERLAY (Tampilkan log di layar)
-- ==============================================
...
-- Set posisi awal floating log
logFloating.Position = UDim2.new(1, -290, 0, 10)

print("✅ R4NzDev EKSTRIM v3.0 berhasil dimuat!")
print("🔥 FITUR EKSTRIM:")
print("   - Fokus HURUF BELAKANG (X, KS, EKS, OO, MP, SF, TT, dll)")
print("   - Deteksi otomatis 1,2,3 huruf")
print("   - Database otomatis dari GitHub")
print("   - LIVE LOG FLOATING dengan COPY")
print("   - Analisa pola sistem Roblox")
print("   - Tombol SEMUA KATA EKSTRIM (aktifkan semua kategori)")
print("📋 Tekan tombol COPY di log untuk menyalin")

-- ==============================================
-- INFO PAGE SEDERHANA (bisa diisi sesuai kebutuhan)
-- ==============================================
...
infoDesc.Size = UDim2.new(1, -20, 0, 200)
