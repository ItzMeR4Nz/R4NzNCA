...
-- =============================================
-- ANALISA UI (DIPINDAHKAN KE ATAS)
-- =============================================
...
-- =============================================
-- SPEED SETTINGS PANEL (DIPERBAIKI)
-- =============================================
...
-- Delay mengetik dengan slider sederhana
...
createAdjustButton("⬇️ -", 110, function()
    ...
    if delayMax < delayMin then delayMax = delayMin + 0.01 end
end)
...
for i, tname in ipairs(themes_list) do
    ...
    themeBtn.MouseButton1Click:Connect(function()
        ...
        for _, btn in pairs(themeDropFrame:GetChildren()) do
            if btn:IsA("TextButton") then
                local prev = btn:FindFirstChildOfClass("Frame")
                if prev then
                    local themeName = btn.Text:gsub("  ", "")
                    prev.BackgroundColor3 = themes[themeName].primary
                end
            end
        end
    end)
...
UserInputService.InputChanged:Connect(function(input)
    if isResizing and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        resizeHandle.Position = UDim2.new(pos.X.Scale, pos.X.Offset + newWidth - 24, 
                                         pos.Y.Scale, pos.Y.Offset + newHeight - 24)
    end
end)
...
-- Update resize handle position setiap frame
RunService.RenderStepped:Connect(function()
